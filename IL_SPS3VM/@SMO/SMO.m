classdef SMO
    ...
    methods(Static = true)
        function obj=SMO(Data,Label,C,C_plus,C_minus,eps,svm_type,ktype,kscale,alpha0)
            global  x y KTYPE KSCALE initUbIn alpha_home sample_length p_term
            KTYPE = ktype;
            KSCALE =kscale;
            if svm_type==1      %C_SVC
        ...
        function [out]=InitialSolution(x,y,C)
            ...
            index_y=find(y~=0);
            alpha0=zeros(length(index_y),1);
            out=SMO(x(index_y,:),y(index_y),C,0,0,fake_zero,1,KTYPE,KSCALE,alpha0);
            out.x=x(index_y,:);
            out.y=y(index_y);
        end
        ...
        function [b,a_size,index_iteration] = quadsmo(alpha0,svm_type)
            ...
            %��ʼ������Ĵ�С������
            [leave_index,active_index,active_size,local_gi]=SMO.DoShrinking(gi,gi_bar,flag_up,flag_low,alpha0,initUbIn,y,index_home,svm_type);
            shrink_count=1;
        ...
        function [gi]=Reconstruct_gradient(local_gi,gi_bar,active_index,svm_type)
            ...
            if svm_type==1
                gi(other_active_index,:)=gi_bar(other_active_index)-p_term;  %��ʼ���ǻ����gi
            else
                gi(other_active_index,:)=gi_bar(other_active_index)-p_term(other_active_index);  %��ʼ���ǻ����gi
            end
            if svm_type==3
