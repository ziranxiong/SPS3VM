...
%%%%%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~initlowIn
        function obj=SemiSVM(Data,Label,initLowIn,initUbIn,alpha0)
            global  x y KTYPE KSCALE   alpha_home sample_length p_term;
%%%%%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%          %%%扩展x和y还有alpha0
%                 size_training=length(Label);               
%          nonLabel_index=1:size_training;
%          Label_index=find(~Label==0);
%          nonLabel_index(Label_index)=[];
%                 x=[Data(Label_index,:);Data(nonLabel_index,:);Data(nonLabel_index,:)];
%                 y=[Label(Label_index);-ones(length(nonLabel_index),1);ones(length(nonLabel_index),1)];
%             size_training=length(y);%n
%             max_low=max(initLowIn);
%             min_up=min(initUbIn);
%             temp_alpha=(max_low+min_up)/2;
%             if  alpha0==0
%                 if mod(size_training,2)==0
%                     alpha0(1:size_training/2,1)=temp_alpha;
%                     alpha0(size_training/2+1:size_training,1)=-temp_alpha;
%                 else
%                     alpha0((1:size_training-1)/2,1)=temp_alpha;
%                     alpha0((size_training-1)/2+1:size_training-1,1)=-temp_alpha;
%                     alpha0(end,1)=0;
%                 end
%             end
% %%%%%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~还要扩展up和low
%                 p_term=y;
%                 initLowIn=[C_low(Label_index);C_low(nonLabel_index);C_low(nonLabel_index)];
%                 initUbIn=[C_up(Label_index);C_up(nonLabel_index);C_up(nonLabel_index)];
%                 [b,active_size,iteration]=SemiSVM.quadsmo(alpha0);
%                 obj.pms.c_low=C_low;
%                 obj.pms.c_up=C_up;
                x=Data;
                y=Label;
...
%             local_obj.b=b;
%             outflag=SemiSVM.TestKKT(local_obj,initLowIn,initUbIn);
%             if outflag==1
...
%             %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function [b,a_size,index_iteration] = quadsmo(alpha0,initLowIn,initUbIn)
            
%%%%%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~initLowIn
            global fake_zero x y max_iteration_smo epsilon sample_length index_home shrink_state...
                alpha_home flag_up flag_low cache_size  regrad_count cache_memory p_term 
...
%%%%%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~I_up & I_low & 判别函数 & gi_bar
            ...
            Q_LnonZero=SemiSVM.Kernel(x,x(alpha0_nonZero_index,:)).*(y*y(alpha0_nonZero_index)');
...
%                 fprintf('%d',index_iteration);
                if isempty(cache_index1) && isempty(cache_index2)
                    if used_cache==cache_size
                        initQAB=SemiSVM.Kernel(local_x,(local_x(local_index,:))).*(local_y*(local_y(local_index))');%n*2
                    ...
                    elseif used_cache==cache_size-1
                        initQAB=SemiSVM.Kernel(local_x,(local_x(local_index(1),:))).*(local_y*(local_y(local_index(1)))');
                        ...
                        initQAB(:,2)=SemiSVM.Kernel(local_x,(local_x(local_index(2),:))).*(local_y*(local_y(local_index(2)))');
...
%                 fprintf('%e\n ',fval0-fval1);
            ...
            %%%%%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~local_flag_up & local_flag_low & local_gi & gi_bar
                ...
                if active_size==sample_length
                    ...
                    if ~(uij(1)==update_uij(1))
                        ...
                        Q_Li(other_active_index,:)=SemiSVM.Kernel(x(other_active_index,:),(x(active_index(local_index(1)),:))).*(y(other_active_index)*(y(active_index(local_index(1))))');
                        if uij(1)
                            gi_bar=gi_bar-old_alpha(1)*Q_Li;
                        else
                            gi_bar=gi_bar+new_alpha(1)*Q_Li;
                    ...
                    if ~(uij(2)==update_uij(2))
                        ...
                        Q_Li(other_active_index,:)=SemiSVM.Kernel(x(other_active_index,:),(x(active_index(local_index(2)),:))).*(y(other_active_index)*(y(active_index(local_index(2))))');
                        if uij(2)
                            gi_bar=gi_bar-old_alpha(2)*Q_Li;
                        else
                            gi_bar=gi_bar+new_alpha(2)*Q_Li;
...
%                 %%%%%%%%%%%%%%%%%%%%%%%%
        ...
        function [gi]=Reconstruct_gradient(local_gi,gi_bar,active_index)
                   %%%%%%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~    initLowIn     
            global x y p_term initLowIn initUbIn fake_zero sample_length alpha_home regrad_count;
            ...
            if length(other_active_index)>reSize
                ...
                for i=1:reGroup
                    QPiAF=SemiSVM.Kernel(x(other_active_index(((i-1)*reSize+1):i*reSize),:),x(free_index,:)).*(y(other_active_index(((i-1)*reSize+1):i*reSize))*(y(free_index))');
                ...
                if i*reSize<length(other_active_index)
                    QPiAF=SemiSVM.Kernel(x(other_active_index(i*reSize+1:end),:),x(free_index,:)).*(y(other_active_index(i*reSize+1:end))*(y(free_index))');
                ...
                QNAf=SemiSVM.Kernel(x(other_active_index,:),(x(free_index,:))).*(y(other_active_index)*(y(free_index))');
...
%             initAlpha=[opitimal_alpha1; y2*sum_two_alpha-yy*opitimal_alpha1];
        ...
        function out_KKT=TestKKT(os,C_low,C_up)
            ...
            if zero<fake_zero2 && zero>-fake_zero2
                local_tmp=SemiSVM.SubKKT(alpha,local_g,C_low,C_up);
                if local_tmp==1
                    out_KKT=1;
                end
            end
        end
        function out_KKT=SubKKT(alpha,local_g,C_low,C_up)
            global fake_zero2
            out_KKT=1;
...
%                             break;
        ...
        function [LowBoud,UppBound]=ComputerCCstar(x,y,label_size,os)
            ...
            x([1:label_size],:)=[];
            y([1:label_size])=[];
            Q=SMO.Kernel(x,os.x).*(y*os.y');
            local_Q=[y Q];
            local_alpha=[os.b;os.alpha];
            local_f=local_Q*local_alpha;
            mu=C_star*(local_f<0);
            low_tmp=(-mu).*(y>0)+(mu-C_star).*(y<0);
            upp_tmp=(C_star-mu).*(y>0)+(mu).*(y<0);
